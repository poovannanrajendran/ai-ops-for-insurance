...
import { StakeholderCommsLogo } from "@/components/stakeholder-comms-logo";
import { demoSamples } from "@/lib/demo-samples";
import type { StakeholderCommsInsight } from "@/types/stakeholder-comms";
...
const appName = "Stakeholder Comms Drafter";
...
function commsAppearance(state: StakeholderCommsInsight["summary"]["status"] | null) {
...
export default function Page() {
  ...
  const [commsText, setCommsText] = useState(demoSamples[0].commsText);
  ...
  const commsState = commsAppearance(result?.analysis.summary.status ?? null);

  async function runAnalysis() {
    const startedAt = globalThis.performance.now();
    const response = await fetch("/api/stakeholdercomms/analyze", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ commsText, sourceLabel, question })
    });

    const data = (await response.json()) as AnalyzeResponse | { error: string };
    if (!response.ok || "error" in data) {
      setResult(null);
      setError("error" in data ? data.error : "Stakeholder comms analysis failed.");
      return;
    }
  ...
  function handleFileSelection(event: React.ChangeEvent<HTMLInputElement>) {
    ...
    reader.onload = () => setCommsText(typeof reader.result === "string" ? reader.result : "");
    ...
    <main className="min-h-screen px-6 py-10 md:px-10 md:py-14">
      <div className="mx-auto flex max-w-7xl flex-col gap-8">
        <section className="overflow-hidden rounded-[34px] border border-[var(--hero-border)] bg-white/90 p-6 shadow-[0_24px_70px_rgba(21,51,72,0.12)] backdrop-blur md:p-8">
          <div className="flex flex-col gap-6 xl:flex-row xl:items-end xl:justify-between">
            <div className="flex flex-col gap-5">
              <div className="flex items-center gap-4">
                ...
                <div className="flex h-16 w-16 items-center justify-center rounded-[22px] bg-[var(--accent-soft)]">
                  <StakeholderCommsLogo className="h-12 w-12" />
                </div>
                <div className="space-y-1">
                  <p className="text-xs font-semibold uppercase tracking-[0.28em] text-[var(--accent)]">Day 28 Internal Tool</p>
              ...
              <div className="space-y-3">
                ...
                <p className="max-w-3xl text-base leading-7 text-slate-600 md:text-lg">
                  Build clear stakeholder updates from structured operational bullets with deterministic extraction, governance checks,
                  and publish-ready message drafts.
                </p>
              </div>
            </div>
            <div className="grid gap-3 sm:grid-cols-2 xl:min-w-[25rem]">
              ...
              <MetaCard label="Comms status" value={commsState.label} dotClass={commsState.dotClass} />
              <MetaCard label="Storage" value={storage.label} dotClass={storage.dotClass} />
              <MetaCard label="Mode" value="Deterministic comms drafting and governance checks" />
            </div>
          </div>
        </section>

        <Card eyebrow="Intake" title="Stakeholder comms capture and prompt">
          <div className="grid gap-5 xl:grid-cols-[1fr_1fr]">
            <div className="space-y-4">
              <p className="text-sm font-semibold text-slate-700">Comms source input</p>
              <div className="rounded-[20px] border border-dashed border-[var(--accent)]/30 bg-white/65 p-5">
                ...
                <p className="mt-3 text-sm leading-6 text-slate-600">Use key-value lines to define communication type, audience, tone, messages, and actions.</p>
            ...
            <div className="space-y-4">
              <p className="text-sm font-semibold text-slate-700">Comms text input</p>
              <div className="rounded-[20px] border border-slate-200 bg-white/65 p-4">
                <p className="text-xs font-semibold uppercase tracking-[0.2em] text-[var(--accent)]">CSV zone</p>
                <textarea className="mt-2 h-[310px] w-full resize-none rounded-[16px] border-0 bg-transparent px-1 py-1 text-sm leading-7 text-slate-800 outline-none shadow-none" id="commsText" onChange={(event) => setCommsText(event.target.value)} value={commsText} />
                ...
                <div className="mt-4 flex flex-col items-center gap-2">
                  <button className="w-full rounded-full bg-[var(--accent)] px-8 py-3 text-base font-semibold text-white disabled:opacity-60" disabled={isPending} onClick={submit} type="button">{isPending ? "Running..." : "Build stakeholder comms"}</button>
                  <p className="text-center text-sm text-slate-500">Runs deterministic extraction, comms drafting, and prompt matching through the app route.</p>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="grid gap-5 xl:grid-cols-2">
          <Card eyebrow="Summary" title="Comms overview">
            <div className="grid gap-3 sm:grid-cols-2">
              ...
              <MetricTile label="Audience" value={result ? result.analysis.summary.audience : "-"} />
