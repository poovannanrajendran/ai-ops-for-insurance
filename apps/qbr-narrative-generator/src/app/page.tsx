...
import { QbrNarrativeLogo } from "@/components/qbr-narrative-logo";
import { demoSamples } from "@/lib/demo-samples";
import type { QbrNarrativeInsight } from "@/types/qbr-narrative";

interface AnalyzeResponse {
  analysis: QbrNarrativeInsight;
  persistence: { status: string; reason?: string };
  processingTimeMs?: number;
  requestId: string;
}

const appName = "QBR Narrative Generator";
...
function performanceAppearance(state: QbrNarrativeInsight["summary"]["performanceState"] | null) {
  if (state === "improving") return { dotClass: "bg-emerald-600", label: "Improving" };
  if (state === "deteriorating") return { dotClass: "bg-red-600", label: "Deteriorating" };
  if (state === "stable") return { dotClass: "bg-amber-500", label: "Stable" };
...
export default function Page() {
  ...
  const [qbrText, setQbrText] = useState(demoSamples[0].qbrText);
  ...
  const perfState = performanceAppearance(result?.analysis.summary.performanceState ?? null);
  const activeFileDisplay = sourceLabel.replace(/\.[a-z0-9]+$/i, "").replace(/qbr|pack/gi, "").trim();

  async function runAnalysis() {
    const startedAt = globalThis.performance.now();
    const response = await fetch("/api/qbrnarrative/analyze", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ qbrText, sourceLabel, question })
    });

    const data = (await response.json()) as AnalyzeResponse | { error: string };
    if (!response.ok || "error" in data) {
      setResult(null);
      setError("error" in data ? data.error : "QBR narrative analysis failed.");
      return;
    }
  ...
  function handleFileSelection(event: React.ChangeEvent<HTMLInputElement>) {
    ...
    reader.onload = () => setQbrText(typeof reader.result === "string" ? reader.result : "");
    ...
    <main className="min-h-screen px-6 py-10 md:px-10 md:py-14">
      <div className="mx-auto flex max-w-7xl flex-col gap-8">
        <section className="overflow-hidden rounded-[34px] border border-[var(--hero-border)] bg-white/90 p-6 shadow-[0_24px_70px_rgba(21,51,72,0.12)] backdrop-blur md:p-8">
          <div className="flex flex-col gap-6 xl:flex-row xl:items-end xl:justify-between">
            <div className="flex flex-col gap-5">
              <div className="flex items-center gap-4">
                ...
                <div className="flex h-16 w-16 items-center justify-center rounded-[22px] bg-[var(--accent-soft)]">
                  <QbrNarrativeLogo className="h-12 w-12" />
                </div>
                <div className="space-y-1">
                  <p className="text-xs font-semibold uppercase tracking-[0.28em] text-[var(--accent)]">Day 26 Internal Tool</p>
              ...
              <div className="space-y-3">
                ...
                <p className="max-w-3xl text-base leading-7 text-slate-600 md:text-lg">
                  Convert core quarterly metrics into board-ready narrative paragraphs, strategic talking points, and
                  deterministic commentary for management packs.
                </p>
              </div>
            </div>
            <div className="grid gap-3 sm:grid-cols-2 xl:min-w-[25rem]">
              ...
              <MetaCard label="Performance state" value={perfState.label} dotClass={perfState.dotClass} />
              <MetaCard label="Storage" value={storage.label} dotClass={storage.dotClass} />
              <MetaCard label="Mode" value="Deterministic QBR narrative synthesis" />
            </div>
          </div>
        </section>

        <Card eyebrow="Intake" title="QBR metrics and prompt">
          <div className="grid gap-5 xl:grid-cols-[1fr_1fr]">
            <div className="space-y-4">
              <p className="text-sm font-semibold text-slate-700">Metric source</p>
              <div className="rounded-[20px] border border-dashed border-[var(--accent)]/30 bg-white/65 p-5">
                ...
                <p className="mt-3 text-sm leading-6 text-slate-600">
                  Paste key-value lines for metric fields (for example <code>loss_ratio_pct</code>, <code>combined_ratio_pct</code>,
                  and <code>renewal_retention_pct</code>) from your quarter pack.
                </p>
            ...
            <div className="space-y-4">
              <p className="text-sm font-semibold text-slate-700">Metrics dataset</p>
              <div className="rounded-[20px] border border-slate-200 bg-white/65 p-4">
                <label className="block text-sm font-semibold text-slate-700" htmlFor="qbrText">
                  QBR metrics block
                </label>
                <textarea
                  ...
                  id="qbrText"
                  onChange={(event) => setQbrText(event.target.value)}
                  value={qbrText}
                />
                ...
                <div className="mt-4 flex flex-col items-center gap-2">
                  <button
                    ...
                    type="button"
                  >
                    {isPending ? "Running..." : "Build QBR narrative"}
                  </button>
                  <p className="text-center text-sm text-slate-500">
                    Runs deterministic metric parsing, narrative drafting, and prompt matching through the app route.
                  </p>
                </div>
              </div>
            </div>
        ...
        <div className="grid gap-5 xl:grid-cols-2">
          <Card eyebrow="Summary" title="Quarter status">
            <div className="grid gap-3 sm:grid-cols-2">
              <MetricTile label="Completeness" value={result ? `${result.analysis.summary.completenessPct}%` : "-"} />
              <MetricTile label="Performance" value={result ? result.analysis.summary.performanceState : "Not run yet"} />
              <MetricTile label="Confidence" value={result ? result.analysis.summary.narrativeConfidence : "-"} />
        ...
        <div className="grid gap-5 xl:grid-cols-2">
          <Card eyebrow="Narrative" title="Executive narrative">
            {result?.analysis.executiveNarrative.length ? (
              <ul className="space-y-2 text-sm leading-6 text-slate-700">
                {result.analysis.executiveNarrative.map((line, index) => (
                  <li key={`${line}-${index}`}>- {line}</li>
                ))}
              </ul>
            ) : (
          ...
          <Card eyebrow="Board" title="Talking points">
            {result?.analysis.boardTalkingPoints.length ? (
              <ul className="space-y-2 text-sm leading-6 text-slate-700">
                {result.analysis.boardTalkingPoints.map((line, index) => (
                  <li key={`${line}-${index}`}>- {line}</li>
                ))}
              </ul>
            ) : (
