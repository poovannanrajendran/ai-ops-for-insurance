...
import { SanctionsScreeningAidLogo } from "@/components/sanctions-screening-aid-logo";
import { demoSamples } from "@/lib/demo-samples";
import type { SanctionsScreeningInsight, Severity } from "@/types/sanctions-screening";

interface AnalyzeResponse {
  analysis: SanctionsScreeningInsight;
  persistence: {
    status: string;
    reason?: string;
  };
...
const appName = "Sanctions Screening Aid";
...
function screeningStateAppearance(state: SanctionsScreeningInsight["summary"]["screeningState"] | null) {
  if (state === "block") return { dotClass: "bg-red-600", label: "Block" };
  if (state === "review") return { dotClass: "bg-amber-500", label: "Review" };
  if (state === "clear") return { dotClass: "bg-emerald-600", label: "Clear" };
...
export default function Page() {
  ...
  const [screeningText, setScreeningText] = useState(demoSamples[0].screeningText);
  ...
  const gate = screeningStateAppearance(result?.analysis.summary.screeningState ?? null);
  const activeFileDisplay = sourceLabel.replace(/\.[a-z0-9]+$/i, "").replace(/statement|text/gi, "").trim();

  async function runAnalysis() {
    const startedAt = performance.now();
    const response = await fetch("/api/sanctionsscreening/analyze", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ screeningText, sourceLabel, question })
    });

    const data = (await response.json()) as AnalyzeResponse | { error: string };
    if (!response.ok || "error" in data) {
      setResult(null);
      setError("error" in data ? data.error : "Sanctions screening analysis failed.");
      return;
    }
  ...
  function handleFileSelection(event: React.ChangeEvent<HTMLInputElement>) {
    ...
    reader.onload = () => setScreeningText(typeof reader.result === "string" ? reader.result : "");
  ...
  function loadSample(sampleId: string) {
    ...
    setScreeningText(sample.screeningText);
  ...
  return (
    <main className="min-h-screen px-6 py-10 md:px-10 md:py-14">
      <div className="mx-auto flex max-w-7xl flex-col gap-8">
            ...
            <div className="flex flex-col gap-5">
              <div className="flex items-center gap-4">
                ...
                <div className="flex h-16 w-16 items-center justify-center rounded-[22px] bg-[var(--accent-soft)]">
                  <SanctionsScreeningAidLogo className="h-12 w-12" />
                </div>
                <div className="space-y-1">
                  <p className="text-xs font-semibold uppercase tracking-[0.28em] text-[var(--accent)]">Day 25 Internal Tool</p>
              ...
              <div className="space-y-3">
                ...
                <p className="max-w-3xl text-base leading-7 text-slate-600 md:text-lg">
                  Screen insured and counterparty entities against deterministic sanctions watchlists and produce clear
        ...
        <Card eyebrow="Intake" title="Entity screening details and prompt">
          <div className="grid gap-5 xl:grid-cols-[1fr_1fr]">
            <div className="space-y-4">
              <p className="text-sm font-semibold text-slate-700">Entity panel</p>
              <div className="rounded-[20px] border border-dashed border-[var(--accent)]/30 bg-white/65 p-5">
                <p className="text-xs font-semibold uppercase tracking-[0.2em] text-slate-500">Entity input</p>
                <p className="text-xs font-semibold uppercase tracking-[0.24em] text-[var(--accent)]">Upload zone</p>
                <p className="mt-3 text-sm leading-6 text-slate-600">
                  Provide key-value rows for <code>insured_entity</code>, <code>beneficial_owner</code>,{" "}
                  <code>broker</code>, <code>territory</code>, and <code>counterparty</code>.
                </p>
            ...
            <div className="space-y-4">
              <p className="text-sm font-semibold text-slate-700">Screening panel</p>
              <div className="rounded-[20px] border border-slate-200 bg-white/65 p-4">
                <p className="text-xs font-semibold uppercase tracking-[0.2em] text-slate-500">Statement text</p>
                <label className="block text-sm font-semibold text-slate-700" htmlFor="screeningText">
                  Screening statement
                </label>
                <textarea
                  ...
                  id="screeningText"
                  onChange={(event) => setScreeningText(event.target.value)}
                  value={screeningText}
                />
                ...
                <div className="mt-4 flex flex-col items-center gap-2">
                  <button
                    ...
                    type="button"
                  >
                    {isPending ? "Running..." : "Check sanctions screening"}
                  </button>
                  <p className="text-center text-sm text-slate-500">
                    Runs deterministic sanctions list overlap checks through the app route.
                  </p>
                </div>
              </div>
            </div>
        ...
        <div className="grid gap-5 xl:grid-cols-2">
          <Card eyebrow="Summary" title="Screening overview">
            <div className="grid gap-3 sm:grid-cols-2">
              <MetricTile label="Entities screened" value={result ? String(result.analysis.summary.entitiesScreened) : "-"} />
              <MetricTile label="Screening state" value={result ? result.analysis.summary.screeningState : "Not run yet"} />
              <MetricTile label="High-risk matches" value={result ? String(result.analysis.summary.highRiskMatches) : "-"} />
              <MetricTile label="Medium-risk matches" value={result ? String(result.analysis.summary.mediumRiskMatches) : "-"} />
            </div>
          </Card>
          <Card eyebrow="Matches" title="Sanctions proximity register">
            {result?.analysis.matches.length ? (
              <div className="space-y-2">
                {result.analysis.matches.map((match, index) => (
                  <div className="rounded-xl border border-slate-200 bg-white/80 px-3 py-2" key={`${match.listName}-${index}`}>
                    <div className="flex items-center justify-between gap-2">
                      <p className="text-sm font-semibold text-slate-900">{match.listName}</p>
                      <span className={`rounded-full border px-2.5 py-0.5 text-xs font-semibold uppercase ${severityClasses(match.severity)}`}>
                        {match.severity}
                      </span>
                    </div>
                    <p className="mt-1 text-sm text-slate-700">
                      {match.inputEntity} vs {match.matchedEntity} ({match.similarityScore}%)
                    </p>
                    <p className="text-xs text-slate-600">{match.rationale}</p>
                  </div>
                ))}
              </div>
            ) : (
              <p className="text-sm text-slate-500">Proximity matches appear after screening.</p>
            )}
          </Card>
        </div>
          ...
          <Card eyebrow="Briefing" title="Narrative and actions">
            ...
            <section className="mt-4">
              ...
              {result?.analysis.recommendedActions.length ? (
                <ul className="mt-2 space-y-2 text-sm leading-6 text-slate-700">
                  {result.analysis.recommendedActions.map((line, index) => (
                    <li key={`${line}-${index}`}>- {line}</li>
                  ))}
                </ul>
              ) : (
                <p className="mt-2 text-sm text-slate-500">Actions appear after analysis.</p>
              )}
            </section>
          </Card>
