...
import type { CatHeatPoint } from "@/types/cat-event-briefing";
...
const countries = feature(worldTopology as never, worldTopology.objects.countries as never) as unknown as {
  features: unknown[];
};
...
function getAutoZoomTransform(points: CatHeatPoint[]) {
  if (points.length === 0) {
  ...
  const scale = Math.min(Math.max(targetRatio / Math.max(spreadRatio, minSpread), 1), maxZoom);
...
export function WorldEventHeatmap({ points }: { points: CatHeatPoint[] }) {
      ...
      <svg className="h-[320px] w-full" preserveAspectRatio="xMidYMid meet" viewBox={`0 0 ${viewWidth} ${viewHeight}`}>
        <g transform={`translate(${transform.tx} ${transform.ty}) scale(${transform.scale})`}>
          ...
          {points.map((point) => {
            ...
            const [x, y] = coords;
            const radius = 5 + point.intensity * 12;
            ...
            return (
              <g key={point.label}>
                ...
                <circle
                  ...
                  strokeWidth="1.1"
                >
                  <title>{`${point.label} | Relative impact ${Math.round(point.intensity * 100)}%`}</title>
                </circle>
              </g>
            );
          })}
